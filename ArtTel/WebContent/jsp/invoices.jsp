	...
	<c:if test="${event=='MAIN' || event=='SEARCH'}">
		...
		<table class="borderedTable" cellpadding="2" cellspacing="1" >
			...
			<tr class="tableHeader">
				<td/>
				<custom:sortableHeader column="${invoicesTableHeader.columns.documentNumber}" sortUrl="invoices.app?event=sort&amp;sortColumn=" />
				<custom:sortableHeader column="${invoicesTableHeader.columns.clientName}" sortUrl="invoices.app?event=sort&amp;sortColumn=" />
				<custom:sortableHeader column="${invoicesTableHeader.columns.grossAmount}" sortUrl="invoices.app?event=sort&amp;sortColumn=" />
				<custom:sortableHeader column="${invoicesTableHeader.columns.netAmount}" sortUrl="invoices.app?event=sort&amp;sortColumn=" />
				<custom:sortableHeader column="${invoicesTableHeader.columns.createDate}" sortUrl="invoices.app?event=sort&amp;sortColumn=" />
				<custom:sortableHeader column="${invoicesTableHeader.columns.paymentDate}" sortUrl="invoices.app?event=sort&amp;sortColumn=" />
				<custom:sortableHeader column="${invoicesTableHeader.columns.comments}" sortUrl="invoices.app?event=sort&amp;sortColumn=" />
				<custom:sortableHeader column="${invoicesTableHeader.columns.documentStatus}" sortUrl="invoices.app?event=sort&amp;sortColumn=" />
				<custom:sortableHeader column="${invoicesTableHeader.columns.user}" sortUrl="invoices.app?event=sort&amp;sortColumn=" />
				<td></td>
			</tr>
			...
			<c:forEach items="${invoiceList.records}" var="invoice" varStatus="rowStatus">
				<tr class="row${rowStatus.index%2}">
					...
					<td  onclick="submitFormWithParam('edit',${invoice.documentId})" ><c:out value="${invoice.number}"/></td>
					<td  onclick="submitFormWithParam('edit',${invoice.documentId})" ><c:out value="${invoice.clientDesc}"/></td>
					<td  onclick="submitFormWithParam('edit',${invoice.documentId})" ><c:out value="${invoice.grossAmount}"/></td>
					<td  onclick="submitFormWithParam('edit',${invoice.documentId})" ><c:out value="${invoice.netAmount}"/></td>
					<td  onclick="submitFormWithParam('edit',${invoice.documentId})" ><c:out value="${invoice.createDate}"/></td>
					<td  onclick="submitFormWithParam('edit',${invoice.documentId})" ><c:out value="${invoice.paymentDate}"/></td>
					<td  onclick="submitFormWithParam('edit',${invoice.documentId})" style="max-width: 250px"><c:out value="${invoice.additionalComments}"/></td>
					<td  onclick="submitFormWithParam('edit',${invoice.documentId})" style="background-color: ${invoice.status.color}" >
						<c:out value="${invoice.status.desc}"/>
					</td>
					<td  onclick="submitFormWithParam('edit',${invoice.documentId})" ><c:out value="${invoice.user}"/></td>
					<td >
						<c:choose>
							<c:when test="${invoice.editable}">
								<!-- <input type="button" value="  -  " onclick="submitFormWithParamAndConfirmation('delete',${invoice.documentId})"/> -->
								<input type="button" value="Kopiuj" onclick="submitFormWithParam('copy',${invoice.documentId})"/>
								<c:if test="${invoice.status.idn=='PENDING'}">
									<input type="button" value="Rozliczona" onclick="submitFormWithParam('settle_invoice',${invoice.documentId})"/>
								</c:if>
							</c:when>
						</c:choose>
					</td>
				...
				<c:if test="${not selectedInvoice.hasCorrection()}" >
					<td>
						<input type="button" value="Wystaw korektę" 
							onclick="submitFormWithParam('correct',${selectedInvoice.documentId})" />
					</td>
					<td width="10"/>
				</c:if>
