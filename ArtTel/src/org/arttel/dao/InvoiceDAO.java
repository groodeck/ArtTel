...
public class InvoiceDAO extends SortableDataPageFetch  {
	...
	private static final String LAST_INVOICE_NUMBER_QUERY =
			"select i.invoiceNumber from Invoice i, Seller s, User u "
	...
	public Invoice getInvoiceById(final String invoiceId) {
		...
		if (StringUtils.isBlank(invoiceId)) {
			...
			result = (Invoice) em.createQuery("from Invoice i where i.invoiceId = :invoiceId")
					.setParameter("invoiceId", Translator.parseInteger(invoiceId))
					.getSingleResult();
		}
		return result;
	}
	...
	public String save(final Invoice invoice) {
		if (invoice.getInvoiceId() != null && !"".equals(invoice.getInvoiceId())) {
			em.merge(invoice);
		} else {
			em.persist(invoice);
		}
	...
	public void setInvoiceStatus(final String invoiceId, final InvoiceStatus newStatus){
		...
		invoice.setInvoiceStatus(newStatus.getIdn());
	}
}
